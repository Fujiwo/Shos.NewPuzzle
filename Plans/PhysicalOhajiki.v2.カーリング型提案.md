...
- `settings.test.js` (新規 / 3 件想定 / **B+A-3 採用追加**): `localStorage` load / save / default
- `hud.test.js` (新規 / 3 件想定 / **B 採用追加**): パワーゲージ (5.2) / 距離リングバッジ (5.3) / ハンマー HUD (5.4) のロジック

**最終想定**: 約 **141 件** (126 + 15 純増 = -9 gravity 除却 + 9 v2 コア + 6 B 採用追加 + 9 既存拡充)

### 6.3 B 採用に伴う §6.1 マトリクスへの追補 (2026-04-22 確定)

B (5.1〜5.4 を MVP-α 採用) と A-3 (設定保存) は **既存「改修 (大)」モジュール内に吸収可能** であり、改修区分ランクは変動しない。具体的な追加内容:

| モジュール | 追加改修内容 | 区分への影響 |
|---|---|---|
| `src/render/ui.js` | (5.2) パワーゲージ HUD / (5.3) 距離リングバッジ / (5.4) ハンマー HUD / (A-3) 設定パネル | 改修 (大) のまま |
| `src/render/canvas.js` | (5.1) 軌道予測線描画 (摩擦のみ自由飛行軌道) | 改修 (中) のまま |
| `src/main.js` | (A-3) 起動時 `localStorage` ロード / (T キー) ショートカット結線 | 改修 (大) のまま |
| `src/input/keyboard.js` | (T キー) 軌道予測線トグル追加 | 改修 (中) のまま |
...
2. ✅ **解決済 (A-1 / 2026-04-22)**: 投擲位置選択の UI 詳細 → **「ドラッグで連続調整」** (§4.6 参照)
3. ✅ **解決済 (A-3 / 2026-04-22)**: 軌道予測線の表示 ON/OFF → **設定で消せるオプションを設ける** (`localStorage` 保存 / `T` キーショートカット / §4.6 参照)
...
8. **(B 採用追加)** パワーゲージ (5.2) の「適正パワー」帯の定義 — 標的中心への到達速度ベースか、滞留位置ベースか。
...
| M1v2.6 | 軌道予測線描画 (5.1 MVP) | M | M1v2.5 |
| M1v2.7 | HUD 再設計 + パワーゲージ (5.2) + 距離リング (5.3) + ハンマー HUD (5.4) + 設定パネル (A-3) | M+ (1.5 日) | M1v2.3 |
| M1v2.8 | テスト全リファクタ + house/fgz/settings/hud 新規 | M | M1v2.2〜M1v2.7 |
| **小計 (Phase 1v2)** | | **約 9〜10 営業日相当** | — |

**総計**: Phase 0v2 (2〜3 日) + Phase 1v2 (9〜10 日) + Phase 2v2 (3〜4 日) ≒ **約 14〜17 営業日**
...
## §9 セッション進行チェックポイント (2026-04-22 全承認 ✅)

| # | チェックポイント | 状態 | 確定内容 |
|---|---|---|---|
| (A) | §4 推奨案 (案 A) | ✅ 承認 | A-1 ドラッグ連続調整 / A-2 摩擦のみ自由飛行軌道 / A-3 ON/OFF 切替オプション (`localStorage`+`T` キー) |
| (B) | §5 追加提案の採否 | ✅ 承認 | MVP-α = 5.1+5.2+5.3+5.4 / MVP-β = 5.5+5.1完全+5.3完全 |
| (C) | §6 影響範囲マトリクス | ✅ 承認 | テスト 126→141 (+15) / Phase 1v2 = 9〜10 営業日 / 総計 14〜17 営業日 |

→ 次工程: 別セッション (writing-plans スキル) で `Plans/PhysicalOhajiki.開発計画.md` を v2 ピボット計画として書き換え。
